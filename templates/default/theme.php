...
add_action( 'bp_before_member_header', 'bp_cover_default_scripts' );
...
function cover_image_scr($user_id=false){
	...
	<style>
     ...
     $filter .= '<div class="img-profile-data">
			...
			if ( $attachment_id > 0 ){
			$filter .= '<a class="btnc btn-sm"  data-modal-id="popup-cover" title="'.__("Gallery", "bp-cover").'"><i class="dashicons dashicons-format-gallery"></i></a>';
			}
           	if(!empty($image)) {
			$filter .='<a  class="btnc btn-save btn-sm" title="'.__("Save", "bp-cover").'" ><i class="dashicons dashicons-yes"></i></a> <a  class="edit-cover btnc btn-sm" title="'.__("Edit Position", "bp-cover").'"><i class="dashicons dashicons-sort"></i></a><a href="#"  class="btnc btn-remove btn-sm" title="'.__("Delete", "bp-cover").'"><i class="dashicons dashicons-trash"></i></a>';
			}
            $filter .= ' </div></div>';			
			}  
...
function bp_cover_gallery_pop() {
    ...
    if(empty($user_id))
		...
		$author_id =  get_current_user_id();
		if ($user_id == $author_id){
        $filter .= '<div class="js-modalbox"><div id="popup-cover" class=" modal-box" >
                <header> 
...
function cover_group_image_scr(){
		...
		'height' => 90 ) );
				...
				if (is_admin() || $bp->is_item_admin ) { 
					$filter .='<li><input type="radio" name="btn" value="upload-cover"/>
                               <label for="btn"><i class="dashicons dashicons-admin-generic"></i></a></label>
		...
		if(!empty($image)){
		 ...
		 if(empty($image)){	    
						    ...
						    $filter .='	<a href="javascript:void(0)" id="uploadgroup" class="btnc btn-sm" href=""><i class="dashicons dashicons-upload"></i> </a>
									   <input type="file" name="group_filename" id="group_pic" class="uploadFile"  >   ';	
								   
                            $filter .= '<a  class="edit-cover btnc btn-sm" ><i class="dashicons dashicons-sort"></i></a>
								       <a  class="btnc btn-save btn-sm" ><i class="dashicons dashicons-yes"></i></a></div></ul></nav></figcaption></figure></li> '  ;
